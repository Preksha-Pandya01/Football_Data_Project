...
create or replace table intermediate_schema.int_player_profile (
...
BEGIN
    ...
    SELECT
        ...
        player_agent_id,
        player_agent_name,
        to_date(split_part(date_of_death,' (', 1),'DD.MM.YYYY') AS              date_of_death,
        CASE
            WHEN date_of_death IS NOT NULL
                 THEN EXTRACT(year FROM to_date(split_part(date_of_death,' (', 1),'DD.MM.YYYY')) - EXTRACT(year FROM date_of_birth)
